...
struct gcm_key{
  ...
  gcm_key ak()const {return gcm_key(pt,ct,mis,correct,-1);}//access key (ak for short; I'm a lazy typist); eta doesn't matter for access so set it to default so we access the map ok
...
struct par_par{
  ...
  //WARNING: in order to ensure proper setting of h and uverr with qbg:2 and qbg:3
  //always use this set_parameter argument type--or at least after any h:* or uverr:* calls
...
const par_par dlm=par_par(0.0009,4,4,0.0035,"xxuvuvxx",true,true);  

class MMT_Parameters{
 public: 
  MMT_Parameters(par_par varied);
  ...
  vector<float32fixed<18> > z_nominal, z_old;
...
struct finder_entry{
  //make a well-behaved constructor
  finder_entry(bool _is_hit=0, int _clock=-1,const hdst_key& k=hdst_key(),const hdst_info&i=hdst_info());
  ...
  int clock;
  //Finder_Detail stuff:
  hdst_key key;//BC_time,time,gtime,VMM_chip;
  hdst_info info;//int strip,station_eta;
};
...
struct athena_entry{
  //make a well-behaved constructor
  athena_entry(int mul=0, int gg=0, double gt=0, double t=0, const TVector3& trp=TVector3(), const TVector3& stl=TVector3(), const TVector3& stg=TVector3(), double q=0,int str=0,int esta=0);
