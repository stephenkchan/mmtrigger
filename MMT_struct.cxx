...
par_par::par_par(double _h,int xct,int uvct,double uver,const string& set,bool ql,bool dlm,bool qbg,double _qt,std_align mis,std_align cor,bool fill_tab,int cs,const string&pd,const string&tg,bool doover):
  h(_h),ctx(xct),ctuv(uvct),uverr(uver),setup(set),islarge(ql),q_dlm(dlm),genbg(qbg),qt(_qt),misal(mis),corr(cor),fill_val(fill_tab),colskip(cs),pcrep_dir(pd),tag(tg),redo(doover) {}
...
void par_par::set_parameter(const string&par){
  ...
  else if(par.find("redo:")==0)redo=atoi(par.substr(5).c_str());
  else if(par.find("qbg:")==0){
    int bgpar=atoi(par.substr(4).c_str());
    if(bgpar>1){
      double vmm_road=0.445*64/(q_dlm?7583.5:7478.);//hard coded because this is a par_par not MMT_Parameters object; also
      int roads_per_vmm=(int)pow(2.,int(bgpar/2)-1.);
//       map<int,double>vmm_hash={{1,0.003756},{2,0.001878},{4,0.000939}};
//       h=vmm_hash[];uverr=vmm_hash[1];
      h=vmm_road/roads_per_vmm;uverr=vmm_road;
    }
    genbg=(bgpar%2==1);
  }
