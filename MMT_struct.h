...
struct gcm_key{
  ...
  //bgcode 0--3: std (y/n) x wide (std/1/2/4 (roads per vmm for the integers); stored as 0,1,2,3): formula is (width_code)*2 + bg? where width, for non-std corresponds to 2^(width_code-1) roads per VMM
  int qt,bgcode;
};

struct par_par{
  par_par(double _h,int xct=0,int uvct=0,double uver=0,const string& setup="",bool ql=true,bool q_dlm=false,bool qbg=0,double _qt=1.,std_align mis=std_align(0),std_align cor=std_align(0),bool fill_tab=true,int cs=0,const string&pd="",const string&tg="",bool doover=false); 
  par_par();
...
const par_par dlm=par_par(0.0009,3,3,0.0035,"xxuvuvxx",true,true);  
