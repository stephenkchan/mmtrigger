...
string std_align::print(bool detailed)const{
  if(!detailed){
    if(type==1){
      ostringstream stuff;
      for(int i=0;i<parmax();i++)stuff<<(i==0?"":",")<<par_val(i);
      return stuff.str();
    }
    return string("");
  }
...
void par_par::set_parameter(const string&par){
  ...
  else if(par.find("qbg:")==0)genbg=atoi(par.substr(4).c_str());
...
string par_par::print_pars(bool detailed,bool comp) const{
  ostringstream mars;
  if(detailed){
    mars<<"_h"<<h;
    if(!comp)mars<<"_ctx"<<ctx<<"_ctuv"<<ctuv;
    mars<<"_uverr"<<uverr<<"_set"<<setup<<"_ql"<<islarge<<"_qdlm"<<q_dlm<<"_qbg"<<genbg<<"_qt"<<qt;
  }
  else{
    mars<<h;
    if(!comp)mars<<","<<ctx<<","<<ctuv;
    mars<<","<<uverr<<",\""<<setup<<"\","<<islarge<<","<<q_dlm<<","<<genbg<<","<<qt;
  }
  if(!comp)mars<<misal.print(detailed)<<corr.print(detailed);
  return mars.str();
}
...
//   maximum_large_theta = atan((H+h1)/z_nominal.back())-tol;
  ...
  minimum_large_theta = float32fixed<4>(atan(H.getFloat()/z_nominal.front().getFloat())+tol);
  maximum_large_theta = float32fixed<4>(atan(sqrt(pow(H+h1,2)+0.25*pow(w1.getFloat(),2))/z_nominal.back().getFloat())-tol);
...
//   return 0;  
  ...
  int the_bin=ybases.front().size()-1;//if it's over, just keep it...for now
...
void MMT_Parameters::fill_crep_table(const string&dir,const string&tag){
  ...
  int nk=nsimmax_1d();
  ostringstream crep_nom;crep_nom<<dir<<(dir.substr(dir.length()-1)=="/"?"":"/")<<"pcrep"<<bill.print_pars(true)<<"_"<<tag<<".txt";
...
void MMT_Parameters::fill_yzmod(){
  ...
  for(int et=0;et<n_etabins;et++){
	...
	double zflt=z_nominal[x_planes[pl]].getFloat(),x=zflt*tan(theta)*sin(phi),yup=zflt*tan(theta)*cos(phi)-ybases[x_planes[pl]][0].getFloat();
...
// 	zadd-=1.*x*sin(beta)/store_const();yadd+=yup*zadd/zflt/store_const();
	zadd+=(1./(1+tan(beta)*tan(theta)*sin(phi))-1.)*zflt/store_const();
...
//   vector<double> y_slopes;
//   y_slopes.push_back((H+h1*)
  vector<float32fixed<18> > z_hit(z_nominal);
...
//   double m_y_max = (H+h1*cos(angle))/z_hit.front(); //2;
  double m_y_max = (H+h1)/z_hit.front().getFloat(); //2;
  double m_y_min = H.getFloat()/z_hit.back().getFloat();  //
  double m_x_max = (w2.getFloat()/2)/z_hit.front().getFloat(); //2;
  double m_x_min = (-w2.getFloat()/2)/z_hit.back().getFloat(); //-2;

  int n_x = ceil(m_x_max/h_mx.getFloat())+1;
  int n_y = ceil((m_y_max - m_y_min)/h_my.getFloat());
...
string MMT_Parameters::bool_to_hit_str(const vector<bool>&track)const{
  ...
  for(int i=0;i<track.size();i++)ok<<track[i];
  return ok.str();
}
